...
/**
 * Calculate brightness value from a supported CSS color.
...
export function brightnessByColor(color) {
  ...
  const isLAB = color.indexOf('lab') === 0;
  const isOKLAB = color.indexOf('oklab') === 0;
  const isOKLCH = color.indexOf('oklch') === 0;
  const isSRGB = color.indexOf('color(srgb') === 0;
  ...
  if (isOKLAB) {
    const m = color.match(/^oklab\(([\d.]+)(%?)/);
    if (m) {
      const l = parseFloat(m[1]);
      return m[2] === '%' ? (l * 255) / 100 : l * 255;
    }
  }
  if (isOKLCH) {
  ...
  if (isSRGB) {
    const m = color.match(/^color\(srgb\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)/);
    if (m) {
...
export function paint(
  ...
  const brightnessThreshold = elemColor.startsWith('lab(') ? 127.5 : 160;
  const useDarkColors = brightnessByColor(elemColor) >= brightnessThreshold;
  ...
  const sentenceVec4s = new Array((sentencePositions?.length || 0) / 4)
    .fill(0)
